...
Although parts of the code use the terms **SRGAN**, **high resolution**, and **low resolution**, the current implementation is primarily an **image restoration/enhancement system rather than conventional spatial super-resolution**.

Both the degraded input and clean target are resized to:
...
which does not increase the spatial dimensions of the image.

Therefore, the active model learns approximately:
...
Degraded 128×128 Image
       ...
       Generator
          │
          ▼
Restored 128×128 Image
```

rather than a conventional super-resolution transformation such as:
...
The architecture is inspired by SRGAN-style residual learning and GAN-based image reconstruction, while the implemented task focuses on restoring degraded images at the same resolution.

---

## Running the Project

The notebooks were developed for Google Colab and contain Google Drive paths.

To reproduce the project:

1. Upload the notebooks to Google Colab.
2. Mount Google Drive.
3. Update dataset paths to match your own directory structure.
4. Run `Image_data_generation.ipynb` to create paired clean and degraded images.
5. Run `SRDCGAN.ipynb` to train the GAN.
6. Save the trained model.
7. Update the saved-model path in `OUTPUT_GAN.ipynb`.
8. Run the output notebook to generate and evaluate restored images.

A paired dataset structure similar to the following is expected:
...
Corresponding filenames should represent a clean target and its degraded input.
...
- Input and output images remain at the same spatial resolution.
- The code contains Google Drive-specific paths that must be changed before reproduction.
- Model performance can vary because image degradations and training samples are selected randomly.
- Training a GAN can be computationally expensive.
- PSNR and MSE do not fully capture perceptual image quality.
- The project does not currently provide a standalone application or deployment interface.
...
- Implement true 2× or 4× spatial super-resolution.
- Replace `UpSampling2D(size=1)` with actual upsampling layers when super-resolution is required.
- Separate training, validation, and test datasets more explicitly.
- Add SSIM and perceptual quality metrics.
- Add reproducible random seeds.
- Replace hard-coded Google Drive paths with configurable paths.
- Save model checkpoints and training histories automatically.
- Package the pipeline into reusable Python modules.
- Add a simple interface for uploading and restoring images.
